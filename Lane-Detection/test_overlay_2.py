...
def merge_close_contours(contours, threshold=50):
    ...
    merged_contours = [
        cv2.convexHull(c.reshape(-1, 1, 2).astype(np.int32)) for c in clusters
    ]
...
def process_frame(model, frame, transform, resize_width, resize_height):
    ...
    merged_contours = merge_close_contours(contours, threshold=12)  # threshold
    ...
    top10_mask = np.zeros_like(binary_pred_resized)
    cv2.drawContours(top10_mask, weighted_contours, -1, 255, thickness=cv2.FILLED)

    overlay_img = overlay_binary_mask(
        input_img, top10_mask, alpha=0.5, color=(0, 0, 255)
    )
    return overlay_img
