import torch
import cv2
from engine.utils import load_yaml 
...
This script is intended for Web Interface Execution.
It handles interactions via a web interface for running the model.
...
def draw_boxes_on_frame(frame, ped_info, tl_info, lane_info, task_type='all', debug=False):
    ...
    for warning_text in ped_warning_texts:
        cv2.putText(frame, warning_text[0], warning_text[1], cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
    ...
    if len(tl_messages) > 1:
        ...
        cv2.putText(frame, message, (35, 85), cv2.FONT_HERSHEY_TRIPLEX, 3, color, 3, lineType=cv2.LINE_AA)
        ...
        if prev_tl_messages[-1] == prev_tl_messages[-2] and prev_tl_messages[-1] != 'NONE': #ADD for stability
            message = str(prev_tl_messages[-1])             
            if message == 'STOP':
                color = (0, 0, 255)
...
def main(task_type, CFG_DIR, OUTPUT_DIR, video_path, image_path, ped_score_threshold, tf_score_threshold): 
    ...
    if video_path is not None:
        ...
        detect_video(task_type, pedestrian_model, traffic_light_model, lane_model, 
    ...
    if image_path is not None:
        ...
        detect_image(task_type, pedestrian_model, traffic_light_model, lane_model, 
                         image_path, 
                         OUTPUT_DIR, 
                         ped_score_threshold, tf_score_threshold,
                         cfg['pedestrian']['iou_threshold'], 
